...
typedef struct _nj_ctx {
    ...
    //The value of 170 may need to be increased depending on the number of codes there are per block
    //Read http://www.impulseadventure.com/photo/jpeg-huffman-coding.html for more details.
    //The maximum test value for the .bss memory segment seems to be 128,956 bytes.
    nj_vlc_code_t vlctab1[4][256];
    int buf, bufbits;
    int block[64];
    int rstinterval;
    unsigned char *rgb;
...
static int njShowBits(int bits) {
    //This function just takes 16 bits from the block data stream
...
NJ_INLINE void njDecodeDHT(void) {
    ...
    while (nj.length >= 17) {
        ...
        //The tables alternates from DC to AC. DC table size if 29 bytes
        //The AC is 179 bytes
        //cout<<"The DHT table length "<<nj.length<<endl;
       // cout<<"The DHT table is "<<i<<endl;

        //There is four DHTs table in each JPEG file
        //The code lengths are at most 16 bits
        for (codelen = 1;  codelen <= 16;  ++codelen)
            //The first 16 bytes contains the number of codes for each length
            counts[codelen - 1] = nj.pos[codelen];
        njSkip(17);

        vlc1=&nj.vlctab1[i][0];

        //The currcnt is the amount of "codes" that are in each code length segment
        //The spread is the rest of the bits
        for (codelen = 1;  codelen <= 16;  ++codelen) {
            ...
            for (i = 0;  i < currcnt;  ++i) {
                ...
                vlc1->bits=(unsigned char) codelen;
                vlc1->code = code;
                ++vlc1;

            }
            //This njSkip function means that the program has done currcnt codes and is ready to move on
            njSkip(currcnt);
        }

        //The remain just acts as a way to zero the remaining information
...
static int njConvertVLC(nj_vlc_code_t* vlc, int value){
    ...
    for(int b=0;b<y;b++){
        if((value>=lower[b])&&(value<=upper[b])){
...
static int njGetVLC(nj_vlc_code_t* vlc, unsigned char* code) {
    ...
    int value1=0;
    value1=njConvertVLC(vlc,value);
    //The first 16 bits of each block can be used to find where the code and the bits are.
    //The njShowsBits finds out what number of bits falls into the range of the calculated value
    //then it bits shifts that amount
    //cout<<(int)vlc[value].bits<<endl;
    //cout<<value1<<endl;

    int bits = vlc[value1].bits;

    if (!bits) { nj.error = NJ_SYNTAX_ERROR; return 0; }
    njSkipBits(bits);
    value = vlc[value1].code;
...
NJ_INLINE void njDecodeBlock(nj_component_t* c, unsigned char* out) {
    ...
    c->dcpred += njGetVLC(&nj.vlctab1[c->dctabsel][0], NULL);
    //nj.block[0] is the DC value of the block(or the average color value)
    //The rest of the blocks are AC values
    //This first statement multiplies the the decoded pixel(using huffman decoding) with the
    //quantization value
    nj.block[0] = (c->dcpred) * nj.qtab[c->qtsel][0];

    do {
        value = njGetVLC(&nj.vlctab1[c->actabsel][0], &code);
        if (!code) break;  // EOB
